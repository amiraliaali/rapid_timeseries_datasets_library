...
pub fn compute_split_offsets(timesteps: usize, train_prop: f64, val_prop: f64) -> (usize, usize) {
...
pub fn split_classification(
    ...
    match splitting_strategy {
        SplittingStrategy::Random => { split_random(_py, &data, &labels, train_prop, val_prop) }
        SplittingStrategy::Temporal => { split_temporal(_py, &data, &labels, train_prop, val_prop) }
    }
}
